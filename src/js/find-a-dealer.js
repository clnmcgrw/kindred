...
const supportsGeo = 'geolocation' in window.navigator && Modernizr && Modernizr.geolocation;
...
const markerIconUrl = 'https://cdn2.hubspot.net/hubfs/3974799/2018-web/location-marker.png';
...
const getGeocodeUrl = (addr) =>
	'https://maps.googleapis.com/maps/api/geocode/json?address=' + addr + '&key=' + gmapsKey;
const setCoordsInput = (lat, lng) => $coordsInput.val(lat + '|' + lng);
const setErrorMsg = (msg, classname) => $errorBox.html(`<p class="${classname}">${msg}</p>`);
const setOverlayMsg = (msg, classname) => $overlayMsgBox.html(`<p class="${classname}">${msg}</p>`);
...
const searchFormHandler = (event) => {
	...
	const onGeocodeSuccess = (data) => {
			if (data.status === 'OK') {
				...
				setCoordsInput(gdata.geometry.location.lat, gdata.geometry.location.lng);
				doAutoFormSubmission();
			} else {
		...
		onGeocodeErr = (err) => {
			setErrorMsg('Something went wrong, we apoligize and are working to fix it.', 'cs-wtb-err-p');
			openErrorBox();
		};
	$.ajax({
...
const mapMarkerClickHandler = (index, infoWindow, mrkr, scrollto = false) => {
	...
	$resultsListItems.removeClass('is-currently-selected').eq(index).addClass('is-currently-selected');
...
const resultListItemIterator = (i, item, boundsFit = false) => {
	...
	marker.addListener('click', () => mapMarkerClickHandler(i, infoWindow, marker, true));
...
const actionBarHandlers = () => {
	...
	return window.setInterval(() => {
		if (scrolled) {
			const thresh = $resultsList.offset().top - ($siteHeader.outerHeight() + 10);
